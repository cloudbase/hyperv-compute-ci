...
scp -v -r -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" -i $DEVSTACK_SSH_KEY /usr/local/src/hyperv-compute-ci-2016/devstack_vm/* ubuntu@$FLOATING_IP:/home/ubuntu/

set +e
VLAN_RANGE=`/usr/local/src/hyperv-compute-ci-2016/vlan_allocation.py -a $VMID`
...
nohup /usr/local/src/hyperv-compute-ci-2016/jobs/build_devstack.sh $hyperv01_ip $hyperv02_ip > /home/jenkins-slave/logs/devstack-build-log-$ZUUL_UUID.log 2>&1 &
pid_devstack=$!

# Building and joining HyperV nodes
echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv01"
nohup /usr/local/src/hyperv-compute-ci-2016/jobs/build_hyperv.sh $hyperv01 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv01.log 2>&1 &
pid_hv01=$!

echo `date -u +%H:%M:%S` "Started building & joining Hyper-V node: $hyperv02"
nohup /usr/local/src/hyperv-compute-ci-2016/jobs/build_hyperv.sh $hyperv02 > /home/jenkins-slave/logs/hyperv-build-log-$ZUUL_UUID-$hyperv02.log 2>&1 &
pid_hv02=$!
